...
def run_inference(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
    ...
    PATH_TO_RESULTS = os.path.dirname(w)
    ...
    for annotation in tqdm(annotations):
        ...
        # print(image_path)
    #dataset = LoadImages(image_paths, img_size=imgsz, stride=stride, auto=pt)
    # for image_path in dataset:
        if 1:
            bbox_coords = [b['box_coordinates']
                           for b in annotation['bbox_info']]
            true_boxes.append(bbox_coords)

            # image_path = '/media/raghul/Backup Plus/cotton/cropped_data/v2_gd/images/' + image_path
            # image_path = os.path.join('/mnt/ts-cvml-datastore/ts_data/cropped_data/cotton/v2_gd/images/', image_path)
            image_path = os.path.join(source, image_path)
        ...
        boxes, labels, scores = [], [], []
        preds_np = pred[0].cpu().numpy()
        for pred in preds_np:
            x1 = int(pred[0] / float(img_w) * img0_w)
            y1 = int(pred[1] / float(img_h) * img0_h)
            x2 = int(pred[2] / float(img_w) * img0_w)
            y2 = int(pred[3] / float(img_h) * img0_h)
            boxes.append([x1, y1, x2, y2])
            scores.append(float(pred[4]))
            labels.append(int(pred[-1]))
            cv2.rectangle(img0, (x1, y1), (x2, y2), (255, 0, 0), 2)
        if os.path.exists('~/work/projects/'+name+'/out_images/'+image_path.split('/')[-2]) == False:
            os.makedirs('~/work/projects/'+name+'/out_images/' +
                        image_path.split('/')[-2])
        cv2.imwrite('~/work/projects/'+name+'/out_images/'+
                    image_path.split('/')[-2]+'/'+image_path.split('/')[-1], img0)
        #show('Detection', img0)
    ...
    filename = os.path.basename(
        w) + '_' + str(time.time()) + '_test_results.json'
...
def parse_opt():
    ...
    parser.add_argument('--json_path', default=ROOT /
                        'runs/detect', help='enter input json path')
